...
if (tg) {
  ...
  if(tg.isVersionAtLeast&&tg.isVersionAtLeast("6.1"))tg.setHeaderColor("#1a0533");
  if(tg.isVersionAtLeast&&tg.isVersionAtLeast("6.1"))tg.setBackgroundColor("#0d1b4b");
  ...
  if (tg.HapticFeedback) {
    if(tg.isVersionAtLeast&&tg.isVersionAtLeast("6.1"))window.tgHaptic=tg.HapticFeedback;
  }
}
...
function playCrunch() {
  try {
    ...
    // Bandpass filter for cookie-like crunch
...
var NEWS=["Учёные обнаружили, что печенье решает все проблемы!","Курс Cookie к доллару достиг максимума!","Бабушки требуют повышения зарплаты.","Портал в мир печенья открыт!","Машина времени доставила печенье из 3025 года.","Шахтёры нашли жилу белого шоколада.","Банк Cookie предлагает 420% годовых!"];
...
// Save ID tied to Telegram user or fallback
var saveId=tgUserId?"tg_"+tgUserId:"clicker_local";

// Show player name from Telegram
if(tgUser){
  var nameEl=document.getElementById("player-name");
  if(nameEl)nameEl.textContent=(tgUser.first_name||"")+", твои печенья";
}
...
function handleClick(e){
  var earned=game.clickPower;game.cookies+=earned;game.allTimeCookies+=earned;game.totalClicks++;
  ...
  playCrunch();
  if(window.tgHaptic)window.tgHaptic.impactOccurred("light");
...
function setupLongPress(){
  document.addEventListener("touchstart",function(e){var card=e.target.closest("[data-upgrade-id]");if(!card)return;longPressTimer=setTimeout(function(){buyUpgrade(card.dataset.upgradeId,10);longPressTimer=null;if(window.tgHaptic)window.tgHaptic.impactOccurred("heavy")},500)});
...
function buyUpgrade(id,amount){amount=amount||1;var upg=UPGRADES.find(function(u){return u.id===id});if(!upg)return;var count=game.upgrades[id]||0;var totalCost=0;for(var i=0;i<amount;i++)totalCost+=getCost(upg,count+i);if(game.cookies<totalCost)return;game.cookies-=totalCost;game.upgrades[id]=count+amount;game.totalPurchases+=amount;game.cps=calcCps();playCrunch();if(window.tgHaptic)window.tgHaptic.impactOccurred("medium");renderShop();updateUI();checkAchievements();autoSave()}
function buyClickUpgrade(id){var cu=CLICK_UPGRADES.find(function(u){return u.id===id});if(!cu||game.clickUpgrades.indexOf(id)!==-1)return;if(game.cookies<cu.cost)return;game.cookies-=cu.cost;game.clickUpgrades.push(id);game.totalPurchases++;game.clickPower=calcClickPower();if(window.tgHaptic)window.tgHaptic.impactOccurred("medium");renderShop();updateUI();checkAchievements();autoSave()}

function checkAchievements(){for(var i=0;i<ACHIEVEMENTS.length;i++){var ach=ACHIEVEMENTS[i];if(game.achievements.indexOf(ach.id)===-1&&ach.req(game)){game.achievements.push(ach.id);showAchievementToast(ach);renderAchievements();if(window.tgHaptic)window.tgHaptic.notificationOccurred("success")}}}
...
function renderShop(){var shop=document.getElementById("shop");var html='<div class="text-xs text-gray-500 uppercase tracking-widest mb-2 font-bold">Бонусы клика</div>';for(var i=0;i<CLICK_UPGRADES.length;i++){var cu=CLICK_UPGRADES[i];var owned=game.clickUpgrades.indexOf(cu.id)!==-1;var canAfford=game.cookies>=cu.cost;var cls=owned?"opacity-30 cursor-not-allowed":canAfford?"can-afford cursor-pointer":"disabled cursor-not-allowed";html+='<div class="upgrade-card stat-bar rounded-xl p-2 md:p-3 flex items-center gap-2 md:gap-3 border border-transparent '+cls+'" onclick="'+(!owned?"buyClickUpgrade('"+cu.id+"')"  :"")+'">';html+='<div class="text-xl md:text-2xl w-8 md:w-10 text-center">'+cu.icon+"</div>";html+='<div class="flex-1 min-w-0"><div class="font-bold text-xs md:text-sm truncate">'+cu.name+"</div>";html+='<div class="text-[10px] md:text-xs text-gray-400">'+cu.desc+"</div></div>";html+='<div class="text-right flex-shrink-0">';if(owned){html+='<span class="text-green-400 text-[10px] md:text-xs font-bold">куплено</span>'}else{html+='<div class="font-black text-amber-400 text-xs md:text-sm">🍪 '+fmt(cu.cost)+"</div>"}html+="</div></div>"}html+='<div class="text-xs text-gray-500 uppercase tracking-widest mt-3 mb-2 font-bold">Производство</div>';for(var j=0;j<UPGRADES.length;j++){var upg=UPGRADES[j];var count=game.upgrades[upg.id]||0;var cost=getCost(upg,count);var ca=game.cookies>=cost;var c2=ca?"can-afford cursor-pointer":"disabled cursor-not-allowed";html+='<div class="upgrade-card stat-bar rounded-xl p-2 md:p-3 flex items-center gap-2 md:gap-3 border border-transparent '+c2+'" data-upgrade-id="'+upg.id+'" onclick="buyUpgrade(\''+upg.id+'\')" oncontextmenu="event.preventDefault();buyUpgrade(\''+upg.id+'\',10)">';html+='<div class="text-xl md:text-2xl w-8 md:w-10 text-center">'+upg.icon+"</div>";html+='<div class="flex-1 min-w-0"><div class="font-bold text-xs md:text-sm flex items-center gap-1 md:gap-2">'+upg.name;if(count>0)html+=' <span class="bg-amber-500 text-black text-[10px] md:text-xs px-1.5 py-0.5 rounded-full font-black">'+count+"</span>";html+="</div>";html+='<div class="text-[10px] md:text-xs text-gray-400">'+upg.desc+"</div>";if(count>0)html+='<div class="text-[10px] md:text-xs text-green-400">+'+(upg.baseCps*count).toFixed(1)+"/сек</div>";html+="</div>";html+='<div class="text-right flex-shrink-0"><div class="font-black text-amber-400 text-xs md:text-sm">🍪 '+fmt(cost)+"</div>";html+='<div class="text-[10px] md:text-xs text-gray-500">+'+upg.baseCps+"/сек</div></div></div>"}shop.innerHTML=html}

function updateUI(){document.getElementById("cookie-count").textContent=fmt(game.cookies);document.getElementById("header-total").textContent=fmt(game.allTimeCookies);document.getElementById("header-cps").textContent=game.cps.toFixed(1);var sc=document.getElementById("stat-clicks");if(sc)sc.textContent=fmt(game.totalClicks);var sp=document.getElementById("stat-per-click");if(sp)sp.textContent=fmt(game.clickPower);var ss=document.getElementById("stat-cps");if(ss)ss.textContent=game.cps.toFixed(1);var st=document.getElementById("stat-total");if(st)st.textContent=fmt(game.allTimeCookies);document.getElementById("click-power").textContent=fmt(game.clickPower);document.getElementById("cps-display").textContent=game.cps.toFixed(1);var mc=document.getElementById("m-clicks");if(mc)mc.textContent=fmt(game.totalClicks);var mp=document.getElementById("m-perclick");if(mp)mp.textContent=fmt(game.clickPower);var mt=document.getElementById("m-total");if(mt)mt.textContent=fmt(game.allTimeCookies)}

var newsIndex=0;function updateNewsTicker(){var el=document.getElementById("news-ticker");if(!el)return;el.textContent=NEWS[newsIndex%NEWS.length];newsIndex++}setInterval(updateNewsTicker,4000);
function gameTick(){var now=Date.now();var dt=(now-lastTick)/1000;lastTick=now;if(game.cps>0){var earned=game.cps*dt;game.cookies+=earned;game.allTimeCookies+=earned;updateUI();checkAchievements()}}setInterval(gameTick,50);setInterval(renderShop,1000);
